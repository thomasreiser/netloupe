...
- While the TUI is running, `providers::update::run_background_updater` refreshes that same cache automatically on `[hosting].range_update_interval` (default 24h), calling the same `providers::update::update_all` the `update-data` command uses -- one download/caching implementation either way. Each refresh reloads the provider database and re-evaluates the Hosting pane for every open tab. Headless `check` doesn't run this updater; it only ever reads whatever is already cached (or the bundled snapshot).
- The status line's bottom-right corner always shows the age of the range data actually in use (`Ranges: 3h old`, or `... (bundled snapshot)` when nothing has ever been downloaded), next to the same GeoLite2 indicator; it turns into a `⚠` warning once that age exceeds `[hosting].max_data_age` (default 30 days, configurable). `d` opens a popup (`Mode::DataInfo`) listing every cached range-list file and GeoLite2 edition -- provider/edition, source URL, size, and when it was last written -- for both the ranges cache and `crate::geoip`'s.
...
- `[hosting]`: enabled providers, confidence thresholds, max data age before warning, `range_update_interval` (how often the provider range-list cache refreshes in the background, see Data lifecycle above), and an extra signature directory for user-defined providers (e.g. an internal company IP range)
...
| `d` | Show the data-info popup: what was downloaded, when, from where, and how big it is |
